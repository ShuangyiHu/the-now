...
def read_recent_logs(n: int = RECENT_FOR_PROMPT) -> list[str]:
    ...
    # Filter out separator lines and blank lines
    entries = [line for line in raw if line.strip() and not line.startswith("─")]

    # Return the most recent n entries (file is written in chronological order)
    return entries[-n:]
...
def append_log(affirmation_text: str, ctx: dict) -> None:
    ...
    # Load existing entries, stripping separators and blanks
    if LOG_FILE.exists():
        existing = [
    ...
    # Append and trim to the maximum allowed count
    existing.append(new_entry)
    if len(existing) > MAX_LOG_ENTRIES:
        existing = existing[-MAX_LOG_ENTRIES:]

    # Write back with visual separators between entries for human readability
    separator = "\n─────────────────────────────────────────\n"
...
def get_time_context() -> dict | None:
    ...
    Returns None if the current time falls outside the active window (9AM-9PM PST).
    ...
    hour = now.hour
    slot = now.minute // 30  # 0 for the first half, 1 for the second half of the hour
...
def get_language(slot_label: str) -> dict:
    """
    Determine the output language based on the slot number.
    Even slot numbers → English, odd slot numbers → Chinese.
    This ensures strict alternation across consecutive 30-minute windows.
    """
...
# ─────────────────────────────────────────────────────────────────────────────
# Deduplication — build the "do not repeat" section from recent log entries
# ─────────────────────────────────────────────────────────────────────────────

def build_no_repeat_section() -> str:
    """
    Read recent log entries and format them as a deduplication constraint
    to be injected into the system prompt.
    Returns an empty string on the very first run when no log exists yet.
    """
...
def build_system_prompt(ctx: dict) -> str:
    """
    Assemble the full system prompt from all context components:
    time period, language, personal situation, and deduplication history.
    """
...
# temperature=1.0 maximizes lexical variety across repeated calls with similar prompts
...
def extract_affirmation_text(result: dict) -> str:
    ...
    that contains actual text content (not just a tool_call block).
...
if __name__ == "__main__":
    ...
    if affirmation_text:
        ...
        print(f"Done. Affirmation logged successfully.")
    else:
        # Non-fatal: the push notification was still sent, logging just failed
